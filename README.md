...
2. **Agent scope:** Local app data checks whether the agent has a narrow, temporary permission to act for that user.

The point is to make delegated AI access easy to explain: a human may have broad access, but the agent still needs a narrow, explicit scope for each sensitive action.
...
- A local agent-scope model for short-lived delegated permissions.
...
- **Neon/Postgres:** stores agents, resources, local agent scopes, and local audit events.
...
- Agent scopes are local database rows, not a WorkOS product object.
...
2. Click **Reset demo**.
3. Click **Test agent access**.
4. Invoice read and summarize should pass.
5. Invoice export should fail because the agent lacks `invoice.export`.
6. Click **Grant invoice.export scope**.
7. Run the access check again.
8. Invoice export should pass, but payroll export should still fail.
9. Use the decision card to see the WorkOS human gate and local agent-scope gate.
10. Use **Open in WorkOS Audit Logs** to inspect the emitted audit trail in WorkOS.
