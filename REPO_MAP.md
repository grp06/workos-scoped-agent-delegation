...
- `app/demo/DemoClient.tsx`: client-side demo controls, state refresh, and page composition.
...
- `app/api/authz/check/route.ts`: direct authorization check endpoint.
...
- To change the human permission model: start in `lib/human-access.ts`, then update `scripts/init-workos-fga.ts`.
