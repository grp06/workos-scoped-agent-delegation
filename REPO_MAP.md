...
- `app/api/demo/reset/route.ts`: resets local agent permissions and audit events to the scripted starting state.
- `app/api/demo/run/route.ts`: runs the scripted finance mission for the signed-in user.
- `app/api/agent/grant-visa/route.ts`: grants the local narrow `invoice.export` permission. The route name is legacy.
- `app/api/agent/visas/route.ts`: returns active local agent permissions. The route name is legacy.
...
- `lib/authz.ts`: central access decision module. It combines WorkOS human access and local agent-permission access.
...
- `lib/visas.ts`: active local agent-permission loading. The filename is legacy.
...
- `lib/demo-data.ts`: local seed data for the fake user, agent, documents, and initial agent permissions.
- `lib/db.ts`: Neon/Postgres SQL client.

## Persistence And Setup

- `db/schema.sql`: database tables for users, agents, resources, agent permissions, and audit events.
...
- To change agent-permission rules: start in `lib/authz.ts`, `lib/demo-data.ts`, and `app/api/agent/grant-visa/route.ts`.
